"use client";

import { useState, useEffect } from "react";
import Link from "next/link";
import { BiListUl, BiX } from "react-icons/bi";

export default function Header() {
  const [scrolled, setScrolled] = useState(false);
  const [mobileNavActive, setMobileNavActive] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      setScrolled(window.scrollY > 100);
    };
    window.addEventListener("scroll", handleScroll);
    handleScroll(); // Check on mount
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  const toggleMobileNav = () => {
    setMobileNavActive(!mobileNavActive);
    document.body.classList.toggle("mobile-nav-active");
  };

  const closeMobileNav = () => {
    setMobileNavActive(false);
    document.body.classList.remove("mobile-nav-active");
  };

  return (
    <header
      id="header"
      className={`header flex items-center fixed top-0 w-full z-[997] transition-all duration-500 ${
        scrolled ? "bg-black/80 shadow-lg py-[10px]" : "bg-transparent py-[20px]"
      }`}
    >
      <div className="container flex items-center justify-between relative">
        <Link href="/" className="logo flex items-center me-auto xl:me-0" onClick={closeMobileNav}>
          <svg
            width="176"
            height="20"
            viewBox="0 0 176 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            xmlnsXlink="http://www.w3.org/1999/xlink"
            className="max-h-[36px]"
          >
            <rect width="176" height="20" fill="url(#pattern0_20_69)" />
            <defs>
              <pattern
                id="pattern0_20_69"
                patternContentUnits="objectBoundingBox"
                width="1"
                height="1"
              >
                <use
                  xlinkHref="#image0_20_69"
                  transform="matrix(0.0026455 0 0 0.0232804 0 -0.000529101)"
                />
              </pattern>
              <image
                id="image0_20_69"
                width="378"
                height="43"
                xlinkHref="data:image/png;base64,"
              />
            </defs>
          </svg>
        </Link>

        <nav
          id="navmenu"
          className={`navmenu hidden xl:block ${
            mobileNavActive ? "mobile-nav-active" : ""
          }`}
        >
          <ul className="flex m-0 p-0 list-none items-center gap-8">
            <li>
              <Link
                href="#about"
                className="text-white/90 hover:text-accent font-pretendard text-[16px] font-normal flex items-center justify-between whitespace-nowrap transition-all duration-300 py-[18px] px-[15px]"
                onClick={closeMobileNav}
              >
                판크로스
              </Link>
            </li>
            <li>
              <Link
                href="#services"
                className="text-white/90 hover:text-accent font-pretendard text-[16px] font-normal flex items-center justify-between whitespace-nowrap transition-all duration-300 py-[18px] px-[15px]"
                onClick={closeMobileNav}
              >
                서비스
              </Link>
            </li>
            <li>
              <Link
                href="#portfolio"
                className="text-white/90 hover:text-accent font-pretendard text-[16px] font-normal flex items-center justify-between whitespace-nowrap transition-all duration-300 py-[18px] px-[15px]"
                onClick={closeMobileNav}
              >
                브랜드
              </Link>
            </li>
            <li>
              <Link
                href="#contact"
                className="text-white/90 hover:text-accent font-pretendard text-[16px] font-normal flex items-center justify-between whitespace-nowrap transition-all duration-300 py-[18px] px-[15px]"
                onClick={closeMobileNav}
              >
                창업문의
              </Link>
            </li>
          </ul>
        </nav>

        <div className="flex items-center ms-auto xl:ms-0 gap-4">
          <Link
            className="btn-getstarted text-white border-2 border-accent px-[30px] py-[8px] rounded-[4px] transition-all duration-300 hover:bg-accent hover:text-white text-[14px] whitespace-nowrap hidden sm:inline-block ms-[15px]"
            href="#contact"
          >
            문의하기
          </Link>
          <Link
            className="btn-getstarted text-white border-2 border-accent px-[30px] py-[8px] rounded-[4px] transition-all duration-300 hover:bg-accent hover:text-white text-[14px] whitespace-nowrap hidden md:inline-block ms-[15px]"
            href="https://smartstore.naver.com/redfishcakecarriage"
            target="_blank"
          >
            스마트스토어
          </Link>
        </div>

        <i
          className="mobile-nav-toggle xl:hidden text-white text-[28px] cursor-pointer ms-[10px] z-50 relative flex items-center justify-center"
          onClick={toggleMobileNav}
        >
          {mobileNavActive ? <BiX /> : <BiListUl />}
        </i>
      </div>

      {/* Mobile Nav Overlay Style */}
      <style jsx global>{`
        .mobile-nav-active {
          overflow: hidden;
        }
        .mobile-nav-active .navmenu {
          display: block !important;
          position: fixed;
          inset: 0;
          background: rgba(33, 37, 41, 0.8);
          z-index: 9998;
          transition: 0.3s;
        }
        .mobile-nav-active .navmenu ul {
          position: absolute;
          inset: 60px 20px 20px 20px;
          padding: 10px 0;
          margin: 0;
          border-radius: 6px;
          background-color: #ffffff;
          overflow-y: auto;
          transition: 0.3s;
          display: block;
          flex-direction: column;
          align-items: flex-start;
          gap: 0;
        }
        .mobile-nav-active .navmenu ul li {
            width: 100%;
        }
        .mobile-nav-active .navmenu ul a {
          color: #212529 !important;
          font-weight: 500;
          font-size: 17px;
          padding: 10px 20px;
          display: flex;
          justify-content: space-between;
          align-items: center;
        }
        .mobile-nav-active .navmenu ul a:hover, .mobile-nav-active .navmenu ul .active {
            color: #ffc451 !important;
        }
      `}</style>
    </header>
  );
}
